...
class CircularCandidateCreator(override val pointSnapper: PointSnapper): CandidateCreator {
    ...
    override fun createCandidate(conicSection: ConicSection): Stream<SnapCandidate> {
        ...
        return features.flatMap { (f0, f1, f2) ->
            Stream.of(Stream.of(f0, f1, f2), Stream.of(f0, f1, f2), Stream.of(f0, f1, f2)).flatMap { (a, b, n) ->
                ...
                affine.map { SnapCandidate(Array.of(a, b), snapped, it, conicSection.transform(it)) }
            }
        }
    }
}
