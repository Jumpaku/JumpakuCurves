...
class EllipticTest {
    ...
    fun testReference() {
        println("Reference")
        val s = path.resolve("ellipticFsc.json").toFile().readText().parseJson().get().bSpline
        val eps = Array.ofAll(path.resolve("ellipticReference.json").toFile().readText().parseJson().get().array.flatMap { Point.fromJson(it) })
