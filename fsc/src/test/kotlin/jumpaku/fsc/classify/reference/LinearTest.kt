...
class LinearTest {
    ...
    fun testReference() {
        println("Reference")
        val s = path.resolve("linearFsc.json").toFile().readText().parseJson().get().bSpline
        val eps = Array.ofAll(path.resolve("linearReference.json").toFile().readText().parseJson().get().array.map { it.point })
