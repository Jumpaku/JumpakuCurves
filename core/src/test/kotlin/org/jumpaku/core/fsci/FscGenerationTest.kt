...
class FscGenerationTest {
    ...
    fun testExtrapolateFront() {
        ...
        val b = BSpline(Array<Point>(Point.xy(-2.0, 0.0), Point.xy(-1.0, 0.0), Point.xy(0.0, 2.0), Point.xy(1.0, 0.0), Point.xy(2.0, 0.0)), knots)
        val data = Interval(0.5, 3.0).sample(100).map { TimeSeriesPoint(b(it), it) }
        val a = BSplineFitting(2, knots).fit(extrapolateFront(data, 0.5))
    ...
    fun testExtrapolateBack() {
        ...
        val a = BSplineFitting(2, knots).fit(extrapolateBack(data, 0.5))
        bSplineAssertThat(a).isEqualToBSpline(b)
    }
