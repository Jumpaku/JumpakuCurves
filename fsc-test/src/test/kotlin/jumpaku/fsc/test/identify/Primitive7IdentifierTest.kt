...
class Primitive7IdentifierTest {
    ...
    fun testIdentify_C() {
        ...
        for (i in 0..0) {
            val fsc = resourceText("FscC$i.json").parseJson().flatMap { BSpline.fromJson(it) }.get()
    ...
    fun testIdentify_E() {
        ...
        for (i in 0..0) {
            val fsc = resourceText("FscE$i.json").parseJson().flatMap { BSpline.fromJson(it) }.get()
    ...
    fun testIdentify_FC() {
        ...
        for (i in 0..0) {
            val fsc = resourceText("FscFC$i.json").parseJson().flatMap { BSpline.fromJson(it) }.get()
